...
func writeLoginError(ctx *gin.Context, err error) {
	switch {
	case errors.Is(err, service.ErrLoginRejected):
		Failure(ctx, InvalidArgument("code 无效或已过期"))
	case errors.Is(err, service.ErrLoginConfigMissing), errors.Is(err, service.ErrLoginStorage):
